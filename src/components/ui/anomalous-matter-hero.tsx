...
export function GenerativeArtScene() {
  ...
  useEffect(() => {
    ...
    renderer.setSize(currentMount.clientWidth, currentMount.clientHeight || window.innerHeight);
    ...
    const handleResize = () => {
      ...
      camera.aspect = currentMount.clientWidth / (currentMount.clientHeight || 1);
...
export function AnomalousMatterHero({
  title = 'DCAi Neuro-Engine Online',
  subtitle = 'Let an AI trading brain DCA into MultiversX while you sleep.',
  description = 'DCAi watches MultiversX 24/7, learns from on-chain volatility and liquidity, and drip-feeds your capital into the market at AI-optimized moments—then takes profit before the crowd even reacts.'
}: AnomalousMatterHeroProps) {
  return (
    <section
      role='banner'
      className='absolute inset-0 h-screen w-full overflow-hidden bg-[hsl(var(--background))] text-[hsl(var(--foreground))]'
    >
      <Suspense fallback={<div className='h-full w-full bg-[hsl(var(--background))]' />}>
        <GenerativeArtScene />
      </Suspense>
      ...
      <div className='relative z-20 flex h-full flex-col items-center justify-center pb-32 text-center'>
        <div className='animate-fade-in-long max-w-3xl px-4'>
          ...
          <p className='mx-auto mt-6 max-w-xl text-base leading-relaxed text-[hsl(var(--gray-300)/0.85)]'>
            {description}
          </p>
        </div>
      </div>
